...
@cocotb.test()
async def test_float_multi_argmin(dut):
    """cocotb test?"""
    ...
    for i in range(16):
        arr = [5.0 + j for j in range(16)]
        arr[i] = 3.0
        print(arr)
        dut.s_axis_a_tdata.value = make_binary_vector(arr)
...
def runner():
    ...
    sources = [proj_path / "hdl" / "float_multi_argmin.sv"] #grow/modify this as needed.
    ...
    runner.build(
        sources=sources,
        hdl_toplevel="float_multi_argmin",
        always=True,
    ...
    runner.test(
        hdl_toplevel="float_multi_argmin",
        test_module="test_float_multi_argmin",
